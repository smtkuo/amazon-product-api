...
/* 
                    try{
                        writeFile(`checkamazondata.data`, body, {}, () => {})
                    }catch(e){
                        console.log(e)
                    }
 */
    ...
    grabReviews(body) {
        ...
        /**
         * Get star ratings and percentages from histogram
         */
        const histogramRows = $('.histogram .histogram-row-container');
        try {
            histogramRows.each((index, element) => {
                const starText = $(element).find('.a-section.a-spacing-none.a-text-left').text().trim();
                const star = parseInt(starText.split(' ')[0], 10);
                const percentage = $(element)
                    .find('.a-section.a-spacing-none.a-text-right')
                    .text()
                    .trim()
                    .replace(/\s+/g, '');
                
                if (!isNaN(star) && percentage) {
         ...
         */
        try {
            const totalReviews = $('.averageStarRatingNumerical').text();
            this.reviewMetadata.total_reviews = parseInt(totalReviews, 10) || 0;
        } catch (error) {
            this.reviewMetadata.total_reviews = 0;
        }

        /**
         * Process review list
         */
        const reviewsList = $('#cm_cr-review_list .review');
        let scrapingResult = {};

        reviewsList.each((index, review) => {
        ...
        for (let key in scrapingResult) {
            try {
                const dateText = $(`#${key} [data-hook="review-date"]`).text();
                if (dateText) {
                    scrapingResult[key].review_data = dateText;
                    scrapingResult[key].date = this.geo.review_date(dateText);
                }
            } catch (error) {
                continue;
            }
        ...
        /**
         * If product has more than one variant and formatType is set to all_formats
         * then some reviews can be written for specific variants (ASIN)
         */
        for (let key in scrapingResult) {
            try {
                const formatLink = $(`#${key} .review-format-strip a`);
                const href = formatLink.attr('href') || '';
                const asinMatch = href.match(/product-reviews\/(\w+)\//);
                if (asinMatch) {
                    scrapingResult[key].asin.variant = asinMatch[1];
                }
            } catch (error) {
                continue;
            }
        ...
        for (let key in scrapingResult) {
            try {
                scrapingResult[key].name = $(`#${key} .a-profile-name`).text().trim();
            } catch (error) {
                continue;
            }
        }
        ...
        for (let key in scrapingResult) {
            try {
                const ratingText = $(`#${key} [data-hook="review-star-rating"]`).text();
                const rating = parseFloat(ratingText.split(' ')[0]);
                if (!isNaN(rating)) {
                    scrapingResult[key].rating = rating;
                }
            } catch (error) {
                continue;
            }
        ...
        for (let key in scrapingResult) {
            try {
                scrapingResult[key].title = $(`#${key} [data-hook="review-title"]`).text().trim();
            } catch (error) {
                continue;
            }
        }
        ...
        for (let key in scrapingResult) {
            try {
                scrapingResult[key].review = $(`#${key} [data-hook="review-body"]`).text().trim();
            } catch (error) {
                continue;
            }
        }
        ...
        for (let key in scrapingResult) {
            try {
                const verifiedBadge = $(`#${key} [data-reftag="cm_cr_arp_d_rvw_rvwer"]`);
                scrapingResult[key].verified_purchase = verifiedBadge.length > 0;
            } catch (error) {
        ...
        for (let key in scrapingResult) {
            try {
                const imageSection = $(`#${key} .review-image-tile-section`);
                if (imageSection.length) {
                    scrapingResult[key].media = [];
                    imageSection.find('img').each((_, img) => {
                        let mediaUrl = $(img).attr('src');
                        if (mediaUrl && mediaUrl.includes('.jpg')) {
                            mediaUrl = mediaUrl.replace('_SY88', '_SL1600_');
                            scrapingResult[key].media.push(mediaUrl);
                        }
                    });
                } else {
                    scrapingResult[key].media = [];
        ...
        /**
         * Add all processed reviews to collector
         */
        for (let key in scrapingResult) {
